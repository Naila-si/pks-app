...
export default function DetailPKSModal({ pksId, onClose }) {
  ...
  useEffect(() => {
    if (pksId) {
      const pksData = dataService.getPKSById(pksId);
      if (pksData) {
        Promise.resolve().then(() => {
          setPks(pksData);
          const compData = dataService.getCompanyById(pksData.perusahaan_id);
          setCompany(compData);
        });
      }
    }
  ...
  const handleDownloadSimulate = () => {
    alert(
      `Mensimulasikan pengunduhan berkas PKS: ${pks.dokumen_url}\nBerkas PDF aman dienkripsi.`,
    );
  };
