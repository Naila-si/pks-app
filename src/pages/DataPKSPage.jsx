...
export default function DataPKSPage() {
  ...
  // Lakukan pencarian & pemfilteran
  const filteredPKSList = useMemo(() => {
    return enrichedPKSList.filter(p => {
      // 1. Pencarian (Nama Perusahaan atau Nomor PKS)
      const matchesSearch = 
        p.nama_perusahaan.toLowerCase().includes(searchQuery.toLowerCase()) ||
        p.nomor_pks.toLowerCase().includes(searchQuery.toLowerCase());
      
      // 2. Filter Status PKS
      const matchesStatus = statusFilter === 'Semua' || p.status === statusFilter;

      // 3. Filter Jenis PKS
      const matchesJenisPKS = jenisPKSFilter === 'Semua' || p.jenis_pks === jenisPKSFilter;

      // 4. Filter Jenis Objek
      const matchesJenisObjek = jenisObjekFilter === 'Semua' || p.jenis_objek === jenisObjekFilter;

      return matchesSearch && matchesStatus && matchesJenisPKS && matchesJenisObjek;
    });
  }, [enrichedPKSList, searchQuery, statusFilter, jenisPKSFilter, jenisObjekFilter]);
  ...
  const handleExport = () => {
    const filename = `Data_PKS_JasaRaharja_${new Date().toISOString().slice(0, 10)}`;
    const headers = [
      'Nama Perusahaan', 
      'Nomor PKS', 
      'Judul PKS', 
    ...
    // Mengekspor seluruh data hasil filter
    exportToCSV(filteredPKSList, headers, keys, filename);
  };
