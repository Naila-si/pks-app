...
export default function DataPerusahaanPage() {
  ...
  // Status PKS Perusahaan didasarkan pada status PKS miliknya yang paling kritis
  // Prioritas status kritis: Berakhir > Segera Berakhir > Aktif > Tidak Ada PKS
  const enrichedCompanies = useMemo(() => {
    return companies.map(company => {
      // Ambil seluruh PKS milik perusahaan ini
      const companyPKS = pksList.filter(p => p.perusahaan_id === company.id);
      
      let overallStatus = 'Tidak Aktif';
      
      if (companyPKS.length > 0) {
        const statuses = companyPKS.map(p => getPKSStatus(p.tanggal_berakhir));
  ...
  const filteredCompanies = useMemo(() => {
    let result = enrichedCompanies.filter(c => 
    ...
    return result;
  }, [enrichedCompanies, searchQuery, sortOrder]);
  ...
  // Helper mendapatkan inisial nama perusahaan (misal: "PT. Artha Nugraha" -> "AN")
  const getInitials = (name) => {
    if (!name) return 'CO';
    // Hilangkan "PT.", "CV.", "UD.", "PO."
  ...
  const handleExport = () => {
    const filename = `Data_Perusahaan_JR_${new Date().toISOString().slice(0, 10)}`;
    const headers = [
      'Nama Perusahaan', 
      'Pengelola', 
      'Kontak Telepon', 
    ...
    exportToCSV(filteredCompanies, headers, keys, filename);
  };
