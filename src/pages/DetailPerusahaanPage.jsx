...
export default function DetailPerusahaanPage() {
  ...
  useEffect(() => {
    if (id) {
      const compData = dataService.getCompanyById(id);
      if (compData) {
        Promise.resolve().then(() => {
          setCompany(compData);
          // Ambil semua PKS milik perusahaan ini
          const pksData = dataService.getPKS().filter(p => p.perusahaan_id === id);
          setCompanyPKS(pksData);
        });
      }
    }
  }, [id]);
            ...
            <div className="py-6 space-y-4 text-xs font-semibold text-slate-600">
              ...
              <div>
                ...
                <div className="flex items-center gap-2.5 text-slate-700 bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                  ...
                  <span>{company.telepon || 'Belum Diisi'}</span>
                </div>
              </div>
