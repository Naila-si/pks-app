...
export default function NotifikasiPage() {
  ...
  const { notifications, markAsRead, refreshNotifications } = useNotification();
  ...
  const handleActionClick = (notif) => {
    ...
    if (notif.status_pks === 'Segera Berakhir') {
      // Perpanjang -> Arahkan ke Detail Perusahaan
      navigate(`/perusahaan/${notif.perusahaan_id}`);
    } else {
      // Tinjau Data -> Tampilkan Detail PKS Modal
      setSelectedPKSId(notif.pks_id);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {filteredNotifications.some(n => !n.dibaca) && (
          <button
      ...
      <div className="space-y-4">
        {filteredNotifications.length > 0 ? (
          filteredNotifications.map((notif, index) => {
            ...
            const cardBg = notif.dibaca ? 'bg-white/80 opacity-75' : 'bg-white shadow-md shadow-slate-100/40 ring-1 ring-blue-50/50';
